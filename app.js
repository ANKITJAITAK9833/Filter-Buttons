...
function displayItems(menuItems) {
  const sectionCenterArray = menuItems.map(function (item) {
  ...
  sectionCenterArray.join("");
  sectionCenter.innerHTML = sectionCenterArray;
}
  ...
  filterBtns.forEach(function (btn) {
    ...
    btn.addEventListener("click", function (e) {
      ...
      const filteredItems = menu.filter(
        (item) => e.currentTarget.dataset.id === item.category
      );
      displayItems(filteredItems.length ? filteredItems : menu);
    });
  });
}
